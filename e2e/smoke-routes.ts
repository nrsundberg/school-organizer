...
export const tenantAuthedRoutes: RouteSpec[] = [
  ...
  // Plan-gated CAMPUS+ feature; non-CAMPUS / unauthenticated visitors get the
  // admin ErrorBoundary or upgrade card (still 2xx + heading), so "either" is
  // the right expect for a smoke that doesn't seed a CAMPUS session. The
  // destructive companion `/admin/data-delete` is intentionally NOT in the
  // sweep — a smoke run should never touch a destructive POST and the GET-side
  // render is functionally identical to the export page.

...
## [research: 2026-04-27]
- unified-parent-multi-school: One parent login + one sibling list across multiple campuses in the same district, with per-child dismissal plans. Directly counters the "splitting siblings with different dismissal plans" complaint that surfaces in SchoolPass G2 reviews; sibling preference is a routine district policy. Source: docs/research/2026-04-27-schools.md (hypothesis 1).
- intra-district-transfer-flow: Single-button "move student to <other campus>" at the central office that carries guardians, drill room assignments, and dismissal exceptions to the receiving school. No competitor advertises this; intra-district transfers and continuation-rule moves are routine. Source: docs/research/2026-04-27-schools.md (hypothesis 6).
- district-drill-compliance-export: One-button cross-school drill-compliance PDF/CSV pre-formatted for CT SB 298, MN, MI retention rules. Today's process is per-school binders aggregated by hand; rising compliance pressure makes this a renewal hook. Source: docs/research/2026-04-27-schools.md (hypothesis 7).


## [research: 2026-04-28]
- no-app-pickup-fallback: One-time SMS pickup code + printed PIN hang-tag so an authorized pickup (grandparent, neighbor, occasional aunt) can complete a pickup with zero parent-side install. Direct counter to PikMyKid's "every delegate must download the app" model; the 18% of <$30k-income parents without smartphones make this a real failure mode today. Build size: S. Source: docs/research/2026-04-28-schools.md (hypothesis 3).
- parent-app-language-coverage: Translate every parent-facing string into the top 10 K-12 languages (Spanish, Mandarin, Vietnamese, Arabic, Tagalog, Russian, Haitian Creole, Korean, Portuguese, Bengali) with per-guardian language preference threading through all push/SMS/voice fan-outs. PikMyKid's parent app is English/Spanish only — concrete competitor gap. Title VI obligation is OCR-enforceable. Build size: S–M. Source: docs/research/2026-04-28-schools.md (hypothesis 2).
- wcag-aa-baseline-pass: WCAG 2.1 AA audit + fix across parent app, controller, viewer, admin, plus a public VPAT page. DOJ extended the Title II deadline by one year on 2026-04-20, but the standard didn't change — every K-12 RFP over the next 18 months will ask. Adding @axe-core/playwright to the existing smoke sweep gets us a regression-proof floor. Build size: M. Source: docs/research/2026-04-28-schools.md (hypothesis 1).
